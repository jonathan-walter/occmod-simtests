jaw_model<-function() {
    ...
    # parameters about the variance and covariance of occupancy and detection probabilities.
    var.v <- tau2 / (1-(rho^2))
    sigma1 <- 1/sqrt(tau1) 
    sigma2 <- 1/sqrt(tau2)
...
R2WinBUGS::write.model(jaw_model, con="modtext.txt", digits=9)
...
Jarzyna_nested<-function() {
    ...
    for (i in 1:nspp) {
        ...
        for (j in 1:nsite) {
            ...
            # mu.psi[j,i] <- psi[j,i] #i don't yet know why this occurs
            Z[j,i] ~ dbin(psi[j,i], 1)#Z is generally not observed with certainty, instead
            p.st[j,i] ~ dnorm(b, tau.p1) # in this model there is only 1 random effect for detection, instead of random species effect wth random site-level offsets
            #latent Z is the result of 1 bernouli trial from each psi[j,i]
